...
5. Share your reflection on the Sanctuary Message Board:
  ...
  hard: `You are dispatched to the Eastern Paradise virtual sanctuary (https://simulation.cryptgregresearch.org).
...
export function setDispatchDifficulty(level) {
  ...
  if (box) box.textContent = DISPATCH_PROMPTS[level] || DISPATCH_PROMPTS.easy;
   if (btnEasy && btnMedium) {
    ...
    if (btnHard) {
      btnHard.className = level === 'hard' ? 'btn-diff active hard' : 'btn-diff';
    }
  }
  ...
  const btnModalHard = document.getElementById('btnModalDiffHard');
  ...
  if (btnModalEasy && btnModalMedium) {
    ...
    if (btnModalHard) {
      btnModalHard.className = level === 'hard' ? 'btn-diff active hard' : 'btn-diff';
    }
  }
}
