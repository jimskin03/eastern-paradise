...
Repository and local defaults stay fail-closed on **Solana devnet**. `GET /api/chain/config` reports this process configuration only; `live_cluster_proven` is always false, so a code checkout or label is not proof that the public deployment is on mainnet-beta. Mainnet-beta labels appear only when `SOLANA_NETWORK=mainnet-beta` and `SOLANA_ALLOW_MAINNET=true` are set together with a matching mainnet RPC. Clients, collection metadata, and docs must use that endpoint as the canonical public label source. origin/main remains Solana; an unmerged local BSC/EVM experiment is not production truth. See [Chain configuration](docs/chain-config.md) and [Solana land ownership](docs/solana-land-ownership.md).
