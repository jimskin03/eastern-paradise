...
test('deny-by-default roles only grant player capabilities', () => {
  ...
  assert.equal(hasCapability(roles, 'land'), true);
  ...
  assert.equal(hasCapability([], 'spend'), false);
  assert.equal(hasCapability(['unknown'], 'wallet'), false);
  const viewer = resolveRoles({ id: 'agent_x', name: 'Seeker' }, { TREASURY_VIEWER_AGENT_IDS: 'agent_x' });
  assert.equal(hasCapability(viewer, 'wallet'), true);
  assert.equal(hasCapability(viewer, 'mint'), false);
  ...
  assert.equal(hasCapability(operator, 'burn'), true);
  assert.equal(hasCapability(operator, 'reconciliation'), true);
  assert.equal(hasCapability(operator, 'spend'), true);
  const admin = resolveRoles({ id: 'agent_x', name: 'Seeker' }, { TREASURY_ADMIN_AGENT_IDS: 'Seeker' });
  assert.equal(hasCapability(admin, 'mint'), true);
  assert.equal(hasCapability(admin, 'spend'), true);
});
...
test('2T-2 ledger auth, pagination, idempotency, rate-limit, audit, and write denial', async (t) => {
  ...
  const unauthBalance = await request(server, `/api/economy/balance?agent_id=${a.id}`);
  assert.equal(unauthBalance.status, 401);

  const unauthHistory = await request(server, `/api/economy/transactions?agent_id=${a.id}`);
  ...
  const foreignHistory = await request(server, `/api/economy/transactions?agent_id=${b.id}`, {
    headers: { Authorization: `Bearer ${a.api_key}` }
  });
  assert.equal(foreignHistory.status, 403);
  assert.equal(foreignHistory.data.error_code, 'FORBIDDEN');
  ...
  assert.match(page1.data.transactions[0].id, /^tx_/);
  ...
  const burnDisabled = await request(server, '/api/economy/burn', {
    method: 'POST',
    headers: { Authorization: `Bearer ${a.api_key}` },
    body: { amount: 1 }
  });
  ...
  assert.equal(burnDisabled.status, 403);
  assert.equal(burnDisabled.data.error_code, 'TREASURY_WRITES_DISABLED');
  assert.ok(EconomyManager.getBalance(a.id).merit_balance >= 40);
  ...
  const conflict = await request(server, '/api/economy/transfer', {
    method: 'POST',
    headers: { Authorization: `Bearer ${a.api_key}`, 'Idempotency-Key': key },
    body: { recipient_id: b.id, amount: 4, memo: 'ctrl-other' }
  });
  assert.equal(conflict.status, 409);
  assert.equal(conflict.data.error_code, 'IDEMPOTENCY_CONFLICT');
  assert.equal(EconomyManager.getBalance(b.id).merit_balance, 3);
  ...
  assert.equal(limited.data.error_code, 'RATE_LIMIT_EXCEEDED');
  ...
  assert.throws(() => {
    db.prepare('UPDATE economy_audit SET result = ? WHERE id = ?').run('tampered', audit[0].id);
  });
  assert.throws(() => {
    db.prepare('DELETE FROM economy_audit WHERE id = ?').run(audit[0].id);
  });
});
